 ...
 */
module.exports = function loadConfig(config, servers, keys, objKey, timeout, rootObj, cache) {
  ...
  if (_.keys(inputs).length > 0) {
    ...
    if (response.success) {
      ...
      if(cache && cache.enabled){
        cache.save.call(cache, response.data);
      }
    } else if (cache && cache.enabled) { // 启用缓存
