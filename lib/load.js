...
function loadFromServer(servers, inputs, timeout) {
  ...
  if (response.success) {
    if (response.warn && _.keys(response.warn).length > 0) {
      ...
      sails.log.debug('load-warn: %j', response.warn);
    }
 ...
 */
module.exports = function loadConfig(config, servers, keys, objKey, timeout, rootObj, cache, watchConfig) {
  ...
  extract(inputs, '', config, keys, objKey, 0, rootObj);
